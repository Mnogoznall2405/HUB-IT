...
from {{auth_module}} import {{required_auth_dependency}}
from {{user_model_module}} import User
from {{models_module}} import (
    {{ResourceName}},
    {{ResourceName}}Create,
    {{ResourceName}}Update,
)
from {{service_module}} import {{ResourceName}}Service

router = APIRouter(prefix="{{router_prefix}}", tags=["{{resource_plural}}"])
...
async def list_{{resource_plural}}(
    ...
    current_user: User = Depends({{required_auth_dependency}}),
