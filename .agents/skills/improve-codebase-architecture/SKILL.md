...
description: Find evidence-based module refactoring opportunities when architectural improvement is requested.
---
...
When the candidate has not been selected, do NOT propose interfaces yet. Ask the user: "Which of these would you like to explore?" If the user or an authorized parent workflow already selected the candidate, reuse that selection without asking again.

### 3. Grilling loop

Once a candidate is selected, reuse resolved design decisions and ask only about material unresolved choices. For an explicitly requested grilling session, drop into a grilling conversation. Walk the design tree with them — constraints, dependencies, the shape of the deepened module, what sits behind the seam, what tests survive.
