"""Add durable Hub task attachment Office-preview queue.
...
def upgrade() -> None:
    if _scope() == "chat":
        return
    schema = _schema()
    table_name = "hub_task_attachment_previews"
    ...
    op.create_table(
        ...
        sa.Column("ready_at", sa.Text(), nullable=True),
        sa.ForeignKeyConstraint(
            ["attachment_id"],
            [f"{schema + '.' if schema else ''}hub_task_attachments.id"],
            ondelete="CASCADE",
        ),
...
def downgrade() -> None:
    ...
    if not _has_table(schema, table_name):
        return
    op.drop_index("idx_hub_task_attachment_previews_task", table_name=table_name, schema=schema)
    op.drop_index("idx_hub_task_attachment_previews_lease_expires", table_name=table_name, schema=schema)
    op.drop_index("idx_hub_task_attachment_previews_status_next_attempt", table_name=table_name, schema=schema)
