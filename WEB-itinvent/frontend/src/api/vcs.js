...
export const vcsAPI = {
    ...
    downloadVncRegFile: () => {
        ...
        const ps1Script = [
            ...
            '    try {',
            '        Add-Content $logPath "FETCHING LAUNCH FILE"',
            '        Invoke-WebRequest -UseBasicParsing -Uri $launchUrl -OutFile $tmp',
            '        Add-Content $logPath "LAUNCHING TOKEN FILE"',
            '        Start-Process -FilePath $viewer -ArgumentList "-optionsfile=`"$tmp`""',
            '    } catch {',
            '        Add-Content $logPath "LAUNCH TOKEN FAILED"',
            ...
            '            Add-Content $logPath "CLEANUP OK"',
            '            break',
            '        } catch { }',
            '    }',
            '    if (Test-Path $tmp) { Add-Content $logPath "CLEANUP SKIPPED" }',
            '} else {',
