...
export const MAIL_AI_ENABLED_STORAGE_KEY = 'mail_ai_enabled';

export const MAIL_AI_DISCLOSURE_TEXT = (
...
export function isMailAiEnabled({
  storage,
  envValue = import.meta.env.VITE_MAIL_AI_ENABLED,
} = {}) {
  const resolvedStorage = storage === undefined
    ? (typeof localStorage === 'undefined' ? null : localStorage)
    : storage;
  try {
    const stored = resolvedStorage?.getItem?.(MAIL_AI_ENABLED_STORAGE_KEY);
...
export function setMailAiEnabled(enabled, { storage } = {}) {
  const resolvedStorage = storage === undefined
    ? (typeof localStorage === 'undefined' ? null : localStorage)
    : storage;
  const next = Boolean(enabled);
  try {
    resolvedStorage?.setItem?.(MAIL_AI_ENABLED_STORAGE_KEY, next ? 'true' : 'false');
  } catch {
    // ignore storage access errors
  }
  return next;
