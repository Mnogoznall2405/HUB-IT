...
function AccountWorkspace({ area = 'settings' }) {
  ...
  const blockingErrorNode = blockingError ? (
    <Alert severity="error" onClose={() => setBlockingError('')}>
      {blockingError}
    </Alert>
  ) : null;
  ...
  if (area === 'profile') {
    page = (
      <MainLayout>
        ...
        >
          <Stack spacing={1.1}>
            {blockingErrorNode}
            <ProfileTab user={user} dbOptions={dbOptions} canAccessMail={canAccessMail} />
          </Stack>
        </PageShell>
      </MainLayout>
    );
    ...
    } else if (activeSection === 'departments' && canManageDepartments) {
      adminContent = <DepartmentsTab canManageDepartments={canManageDepartments} />;
    } else if (activeSection === 'ad-users') {
      adminContent = <AdUsers embedded />;
    } else if (activeSection === 'ai-bots' && canManageAiBots) {
      adminContent = (
        <AiBotsAdminSection
          bots={aiBotsState}
    ...
    } else if (activeSection === 'sessions' && canManageSessions) {
      adminContent = (
        <SessionsTab
          sessions={sessions}
    ...
    if (activeSection === 'appearance') {
      settingsContent = (
        <AppearanceTab
          ...
          resolvedMobileNavigationItems={resolvedMobileNavigationItems}
          setMobileBottomNavItems={setMobileBottomNavItems}
