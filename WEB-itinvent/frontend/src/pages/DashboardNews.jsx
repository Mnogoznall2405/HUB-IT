...
function DashboardNews({ newsOnly = false }) {
  ...
  const hubToastAction = useMemo(() => createNavigateToastAction('/dashboard/news', 'Открыть новости'), []);
  ...
  const [mobileView, setMobileView] = useState(newsOnly ? 'announcements' : 'overview');
      ...
      >
        ...
        >
            ...
            <Stack spacing={1.05} data-testid="dashboard-mobile-layout">
              <Card data-testid="dashboard-mobile-header" sx={{ ...getOfficePanelSx(ui, { p: 1, borderRadius: '16px' }) }}>
                <Stack spacing={1}>
                  <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1}>
                    <Box sx={{ minWidth: 0 }}>
                      <Typography sx={{ fontWeight: 600, fontSize: '1rem', lineHeight: 1.15 }}>
                        {newsOnly ? 'Новости' : 'Главная'}
                      </Typography>
                      <Typography variant="caption" sx={{ color: ui.mutedText, display: 'block', mt: 0.25 }}>
                        {newsOnly ? 'Объявления, обязательные сообщения и корпоративная лента.' : 'Важное, новости, почта и мои задачи.'}
                      </Typography>
                    </Box>
                    <OverflowMenu
                      items={mobileHeaderActions}
                      onSelect={handleMobileActionSelect}
                      label={newsOnly ? 'Действия новостей' : 'Действия главной'}
                    />
              ...
              <Stack spacing={0.9}>
                <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={0.9}>
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Box>
                      <Typography sx={{ fontWeight: 600, fontSize: '0.98rem', lineHeight: 1.1 }}>
                        {newsOnly ? 'Новости' : 'Главная'}
                      </Typography>
                      <Typography variant="caption" sx={{ color: ui.mutedText, display: 'block', mt: 0.2 }}>
                        {newsOnly
                          ? 'Корпоративная лента, обязательные сообщения и редактор объявлений.'
                ...
                {!newsOnly ? <Grid container spacing={0.8}>
                ...
                {!newsOnly && coreQuickAccessItems.length > 0 && (
          ...
          <Grid container spacing={1.2} alignItems="stretch" sx={{ flex: 1, minHeight: 0, height: 0, overflow: 'hidden' }}>
            <Grid item xs={12} lg={newsOnly ? 12 : 8} sx={{ display: 'flex', minHeight: 0, height: '100%' }}>
            ...
            {!newsOnly ? <Grid item xs={12} lg={4} sx={{ display: 'flex', minHeight: 0, height: '100%' }}>
