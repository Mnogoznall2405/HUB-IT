...
import DashboardNews, { getAnnouncementReadSecondaryText, normalizeAnnouncementReadsPayload } from './DashboardNews';
...
describe('Dashboard announcement reads helpers', () => {
  ...
  it('renders the production news route without dashboard tabs or task column', async () => {
    installMatchMedia({ mobile: true });

    render(
      <MemoryRouter initialEntries={['/dashboard/news']}>
        <DashboardNews newsOnly />
      </MemoryRouter>,
    );

    expect(await screen.findByText('Новости')).toBeInTheDocument();
    expect(screen.queryByRole('tab')).toBeNull();
    expect(screen.queryByText('Мои задачи')).toBeNull();
    expect(screen.getByRole('textbox', { name: 'Поиск по заметкам' })).toBeInTheDocument();
  });
});
