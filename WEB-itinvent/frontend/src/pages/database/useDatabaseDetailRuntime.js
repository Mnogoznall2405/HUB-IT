...
import { readFirst, toNumberOrNull } from './databaseRecordModel';
import {
...
} from './useEquipmentActFilePreview';
...
export function useDatabaseDetailRuntime({
  ...
  const {
    preview: actFilePreview,
    openingDocNo: detailActOpeningDocNo,
    openActFile,
    closePreview: closeActFilePreview,
  } = useEquipmentActFilePreview({ fallbackInvNo: detailModal?.invNo || '' });
  ...
  useEffect(() => {
    ...
    const item = findEquipmentByInvNo?.(invNo) || detailModal.data || null;

    if (item) {
      setDetailModal((prev) => ({
        ...prev,
        data: prev.data ? { ...item, ...prev.data, ...item } : item,
        loading: false,
      }));
    }

    // Полная карточка уже есть — не ходим в API повторно.
    if (hasFullDetailFields(item)) {
      return undefined;
    }
