...
public sealed class DiagnosticsViewModel
{
    public DiagnosticsViewModel(DesktopDiagnosticsSnapshot snapshot)
    {
        ...
        [
            ...
            new("Память сейчас", FormatBytes(snapshot.Memory?.Current?.Total.PrivateBytes)),
            new("Память HUB Desktop", FormatBytes(snapshot.Memory?.Current?.Host.PrivateBytes)),
            new("Память WebView2", FormatBytes(snapshot.Memory?.Current?.WebViewPrivateBytes)),
            new("WebView2 browser", FormatBytes(snapshot.Memory?.Current?.Browser.PrivateBytes)),
            new("WebView2 renderer", FormatBytes(snapshot.Memory?.Current?.Renderer.PrivateBytes)),
            new("WebView2 GPU", FormatBytes(snapshot.Memory?.Current?.Gpu.PrivateBytes)),
            new("WebView2 utility", FormatBytes(snapshot.Memory?.Current?.Utility.PrivateBytes)),
            new("WebView2 other", FormatBytes(snapshot.Memory?.Current?.Other.PrivateBytes)),
            new("Память active p95", FormatBytes(snapshot.Memory?.Active?.P95PrivateBytes)),
            new("Память active max", FormatBytes(snapshot.Memory?.Active?.MaxPrivateBytes)),
            new("Память background p95", FormatBytes(snapshot.Memory?.Background?.P95PrivateBytes)),
            new("Память background max", FormatBytes(snapshot.Memory?.Background?.MaxPrivateBytes)),
