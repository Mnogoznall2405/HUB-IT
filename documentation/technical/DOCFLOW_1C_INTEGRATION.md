...
|---|---|---|---|
| `acknowledge` | завершение `DMBusinessProcessTask` или `DMBusinessProcessApprovalTaskCheckup`; существующий итог согласования и `returned=false` сохраняются | — | необязателен |
...
| `accept_invitation` | `invitationResult` | `Принято` | необязателен |
| `decline_invitation` | `invitationResult` | `НеПринято` | необязателен |

Для `DMBusinessProcessInvitationTaskInvitation` HUB показывает два отдельных действия: принять приглашение или не принимать. После завершения DMService не возвращает персональный `invitationResult` в карточке выполненного задания, поэтому подтверждением записи служит переход задания в завершённое состояние после успешного `DMUpdate`.

Непринятое задание сначала передаётся в `DMAcceptTasksRequest`, перечитывается и только затем обновляется типизированным `DMUpdateRequest` с `executed=true`. После записи задание перечитывается. Для действий, задающих результат, успех подтверждается по `executed`, дате и точному результату; для `acknowledge` — по завершению без подмены уже существующего итога процесса.
