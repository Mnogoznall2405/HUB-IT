...
def _resolve_default_db_path(data_dir: Path) -> Path:
    primary = data_dir / "local_store.db"
    unified = data_dir / "local_store.unified.db"
    restored = data_dir / "local_store.restored.db"
    if _sqlite_file_is_healthy(unified):
        logger.warning("Using unified local store database at %s", unified)
        return unified
    if _sqlite_file_is_healthy(primary):
        return primary
    if _sqlite_file_is_healthy(restored):
        logger.warning("Using restored local store database at %s", restored)
        return restored
    return primary
...
class SQLiteLocalStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.db_path = Path(db_path) if db_path else _resolve_default_db_path(self.data_dir)
