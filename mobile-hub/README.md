...
Текущий source-кандидат: Expo SDK 57, React Native 0.86, Android package `ru.zsgp.hubit.mobile`, версия `1.1.33`, `versionCode` 35, target/compile SDK 36. Строительные объекты и дополнительные разделы администрирования подготовлены локально; статус публикации проверяется перед релизом.
...
Готовый внутренний preview APK: `dist/hubit-mobile-preview.apk`. Для публикации рядом с HUB Desktop используется `scripts/mobile/publish-apk.ps1`; manifest и версионный APK размещаются под `/desktop-updates/mobile/preview/`. Preview 1.1.28 (30) опубликован 2026-09-08; внешний manifest/APK и SHA-256 проверены. Проверка установки на Android остаётся отдельной. Подробности и rollback: `documentation/technical/MOBILE_HUB_APK_DISTRIBUTION.md`.
...
Состояние сети APK получает от Android `ConnectivityManager`: поле online означает сеть с `NET_CAPABILITY_VALIDATED`, без SSID/IP и другой сетевой идентификации. Для доступа к внутренним ресурсам учитывается также connected: подключённая сеть или VPN не обязаны проходить публичную Android-проверку интернета. После недавнего успешного запроса авторизации потеря сети перепроверяется по окончании 30-секундного grace-интервала; вне этого интервала включается read-only. Более новое событие сети отменяет результат старой перепроверки. После возврата сети приложение восстанавливает офлайн-сессию с backoff. На общей странице Android-настроек можно вручную запросить ограниченный снимок: подтверждён ли интернет, транспорт и metered-режим — без SSID, IP и оператора.

Очистка снимков при выходе блокирует новые записи пользователя и дожидается выполняющихся операций хранилища перед удалением файлов и ключа. Очередь отметок прочтения изменяется последовательно; очистка инвалидирует незавершённую отправку. Карточки оборудования и публикаций изолируют состояние по пользователю и идентификатору записи, а поздние ответы загрузок отбрасываются. Аналитика задач и управление папками почты не загружают данные при известном офлайне; при возвращении сети загрузка возобновляется.
...
Фото и превью Chat отображает через `expo-image`, сохраняя постоянные файлы в существующем `hubit-attachments-v2`; повторный дисковый кэш для этих локальных файлов не создаётся. Обложки ленты и фото структуры компании используют общий `AuthenticatedRemoteImage` с дисковым кэшем `expo-image`: ключ включает пользователя и полный URL с серверной версией изображения, а локальная копия проверяется до запроса токена. Logout и «Очистить кэш файлов» очищают также память и диск `expo-image`. Этот нативный модуль требует пересборки APK. Дисковый кэш превью управляется библиотекой и может вытесняться; он не заменяет постоянное хранение черновиков или предзагрузку всех картинок для офлайна. Счётчик файлов в настройках пока учитывает существующие файловые хранилища, без размера внутреннего кэша `expo-image`.
...
В объектах строительства и направлениях доступна вкладка «Ход работ»: просмотр сводки, плана/факта, сроков, выполнения за смену и комментариев. Дата учёта и архив запрашиваются через существующий `work-progress`, фильтр разделов применяется локально. Ссылки `?tab=work` открывают эту вкладку. Открытые данные сохраняются в зашифрованные снимки отдельно по пользователю, объекту, направлению, дате и архивному режиму. Без сети доступна сохранённая копия; общая подготовка автономного режима заранее строительные работы не загружает. Редактирование плана и журнала остаётся в веб-версии.
...
## Регистрация нового нативного модуля

Каждый пункт обязателен — пропуск любого звена даёт скрытый баг (маршрут открывается, но таб/навбар/deep-link ломается):

1. `app/(shell)/<module>/_layout.tsx` + `index.tsx` — route-стек модуля.
2. `app/(shell)/_layout.tsx` — `Tabs.Screen name="<module>"` (пропуск = пустой таб или падение).
3. `src/navigation/mobileNavItems.ts` — `navigationItems` (path, label, icon, permission, group) + feature-флаг `NATIVE_<MODULE>_ENABLED`.
4. `src/navigation/moduleRegistry.ts` — `hrefForPortalPath`/`openPortalPath` маппинг веб-пути → нативного.
5. `src/navigation/nativeAccountRoutes.ts` — если модуль живёт внутри account/menu области.
6. `src/navigation/systemIntent.ts` — deep-link/push destination при необходимости.
7. `hideBottomNav` паттерны в `app/(shell)/_layout.tsx` — если у модуля есть саброуты, где навбар должен прятаться.
8. Проверка: `resolveActiveBottomNavPath` подсвечивает модуль (prefix-матч саброутов уже общий) или отводит в `/menu`.

Общий выбор базы данных: `src/components/database/NativeDatabasePickerSheet.tsx` — единый bottom-sheet для всех модулей (Инвентарь, Статистика); не добавлять параллельных fullscreen-пикеров.
