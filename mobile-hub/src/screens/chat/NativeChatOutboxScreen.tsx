...
export function NativeChatOutboxScreen() {
  ...
  const run = useCallback(async (row: Row, discard = false) => {
    ...
    try {
      ...
      else await session.retryDelivery(id, () => active.current && scope.current === operationScope
        && access.current.userId === userId && access.current.allowed && access.current.canSend);
    } catch {
