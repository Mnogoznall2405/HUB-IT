...
jest.mock('../../auth/AuthContext', () => ({
  useAuth: () => ({
    user: { id: 1, username: 'mail-performance', role: 'user' },
...
it('does not show the old mailbox rows for an uncached offline search', async () => {
  const view = await render(<NativeMailInboxScreen />);
  await view.findByText('Mail 09');
  mockOfflineMode = true;
  await view.rerender(<NativeMailInboxScreen />);
  await fireEvent.changeText(view.getByTestId('native-mail-search'), 'not cached');
  await waitFor(() => expect(view.queryByText('Mail 09')).toBeNull());
  expect(mockedMailApi.getMailMessages).toHaveBeenCalledTimes(1);
});


it('reuses folder metadata during a search while refreshing messages and counters',async()=>{
 const view=await render(<NativeMailInboxScreen/>);
 await waitFor(()=>expect(view.getByText('Mail 09')).toBeTruthy());
 const count=()=>({messages:mockedMailApi.getMailMessages.mock.calls.length,summary:mockedMailApi.getMailFolderSummary.mock.calls.length,tree:mockedMailApi.getMailFolderTree.mock.calls.length,preferences:mockedConfigApi.getNativeMailPreferences.mock.calls.length});
 expect(count()).toEqual({messages:1,summary:1,tree:1,preferences:1});
 await fireEvent.changeText(view.getByTestId('native-mail-search'),'query');
 await waitFor(()=>expect(mockedMailApi.getMailMessages).toHaveBeenCalledTimes(2));
 expect(count()).toEqual({messages:2,summary:2,tree:1,preferences:1});
 await view.unmount();
});
