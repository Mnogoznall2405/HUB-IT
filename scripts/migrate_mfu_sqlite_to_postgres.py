...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate MFU runtime state from SQLite into PostgreSQL system schema.")
...
def main() -> int:
    ...
    from backend.services.mfu_monitor_service import MfuRuntimeMonitor

    database_url = ensure_app_database_configured()
    initialize_app_schema(database_url)
    MfuRuntimeMonitor(database_url=database_url)
    ...
    print("MFU runtime migration completed:")
    for table_name in TABLES_IN_ORDER:
