...
def test_build_assignee_membership_clause_postgresql():
    clause, params = build_assignee_membership_clause(5, uses_postgresql=True)
    assert "json_array_elements_text" in clause
    assert "assignee_user_ids" in clause
    assert params == [5]


def test_build_assignee_membership_clause_sqlite():
    clause, params = build_assignee_membership_clause(7, uses_postgresql=False)
    assert "json_each" in clause
    assert "assignee_user_ids" in clause
    assert params == [7]
